...
public class FileSplitter {
	 ...
	 * @throws IOException
	 */
	public static Map<String, File> splitFiles(File relatFile, Collection<String> dataSetNames) throws IOException {

		if (dataSetNames.size() == 1) {
			Map<String, File> ret = new HashMap<String, File>();
			ret.put(dataSetNames.iterator().next(), relatFile);
			return ret;
		}
		...
		for (String dataSetName : dataSetNames) {
			File file = new File(relatFile.getParentFile().getAbsoluteFile() + File.separator
					+ FilenameUtils.getBaseName(relatFile.getAbsolutePath()) + "_" + dataSetName + "."
					+ FilenameUtils.getExtension(relatFile.getAbsolutePath()));
		...
		FileInputStream fis;
		try {
			fis = new FileInputStream(relatFile);
			...
			while ((aLine = in.readLine()) != null) {
				...
				for (String dataSetName : dataSetNames) {
					if (split[0].endsWith(dataSetName) || split[1].endsWith(dataSetName)) {
