...
public class SampleImpl implements Sample {
	protected final static HashMap<Integer, Sample> samplesMap = new HashMap<Integer, Sample>();
	...
	public SampleImpl(
			edu.scripps.yates.proteindb.persistence.mysql.Sample sample) {
		hibSample = sample;
	...
	public Tissue getTissue() {
		if (tissue == null) {
			if (hibSample.getTissue() != null) {
				if (TissueImpl.tissuesMap.containsKey(hibSample.getTissue()
						.getTissueId())) {
					tissue = TissueImpl.tissuesMap.get(hibSample.getTissue()
							.getTissueId());
				} else {
	...
	public Organism getOrganism() {
		if (organism == null) {
			final Set<edu.scripps.yates.proteindb.persistence.mysql.Organism> hibOrganisms = hibSample
					.getOrganisms();
			if (hibOrganisms != null) {
				for (edu.scripps.yates.proteindb.persistence.mysql.Organism hibOrganism : hibOrganisms) {
					if (OrganismImpl.organismMap.containsKey(hibOrganism
							.getTaxonomyId())) {
						organism = OrganismImpl.organismMap.get(hibOrganism
								.getTaxonomyId());
					} else {
