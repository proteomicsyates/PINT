...
public class ProteinRetrievalServicesServlet extends RemoteServiceServlet implements ProteinRetrievalService {
		...
		try {
				...
				if (proteinBeansInCache != null) {
					...
					if (!proteinBeansInCache.isEmpty()) {
						...
						for (final ProteinBean proteinBean : proteinBeansInCache) {
							dataSet.addProtein(proteinBean);
						}
			...
			if (defaultQueryIndex == null && !dataSet.getProteins().isEmpty()) {
				...
				log.info("Adding protein beans to cache by project tag '" + projectTagsString + "' in session '"
						+ sessionID + "'");
				ServerCacheProteinBeansByProjectTag.getInstance().addtoCache(dataSet.getProteins(), projectTagsString);
			}
	...
	public static String getProjectTagString(Set<String> projectTags, Integer defaultQueryIndex) {
	 ...
	 * @param sessionID
	 * @param projectTag
	 * @param numQuery   if null, the first query string is loaded (if available)
	 * @return
	 * @throws PintException
	 */
	...
	public FileDescriptor getDownloadLinkForProteinsFromQuery(String sessionID, String queryText,
		...
		logMethodCall(enclosingMethod);
		final String projectTagCollectionKey = SharedDataUtil.getProjectTagCollectionKey(projectTags);
...
//			}
	...
	/**
	 * This method is used in case that the methods in this servlet are used from
	 * another servlet, so the servletContext is not initialized
	 * 
