package edu.scripps.yates.client.ui.wizard;
...
import edu.scripps.yates.client.ui.wizard.view.LazyComposite;
import edu.scripps.yates.client.ui.wizard.view.WidgetDisplay;
import edu.scripps.yates.client.ui.wizard.view.impl.WizardView;
import edu.scripps.yates.client.ui.wizard.view.widget.WizardDeckPanel;

/**
 * Wizard provides a flexible step-by-step GWT widget. It provides its own view,
 * but you can provide your own by extending the {@link Display} interface and
 * passing it to Wizard's constructor.
 * 
...
public class Wizard<C extends WizardContext> extends Composite {
	...
	/**
	 * Wizard.Display defines a Wizard's visual display (or "view" in MVP terms).
	 * {@link Wizard} comes with its own implementation of Display
	 * ({@link WizardView}), but you may provide your own if you wish via
	...
	public interface Display extends WidgetDisplay {
		/**
		 * The {@link Wizard}'s title is handled by an object that implements GWT's
		 * <code>HasHTML</code>.
		 * 
		 * @return implementation of GWT's <code>HasHTML</code> for the {@link Wizard}'s
		 *         title
		 */
		HasHTML getCaption();

		/**
		 * The {@link Wizard} maintains a list of its pages via an implementation of
		 * {@link HasWizardTitles}.
		 * 
		 * @return implementation of {@link HasWizardTitles} for the {@link Wizard}'s
		 *         page titles
		 */
		 ...
		 * {@link HasWizardButtons}. If you implement your own button bar, you can
		 * remove certain buttons (e.g., "Cancel") by returning <code>null</code> in
		 * their getter functions (e.g. {@link HasWizardButtons#getCancelButton()}.
		 * 
		 * @return implementation of {@link HasWizardButtons} for the {@link Wizard}'s
		 *         button bar
		 */
		HasWizardButtons getButtonBar();

		/**
		 * The main content area is implemented with a GWT DeckPanel. To keep this
		 * decision from being forced upon user-created views, we extend DeckPanel into
		 * {@link WizardDeckPanel}, which implements {@link HasIndexedWidgets}. If you
		 * use a panel that doesn't implement GWT's IndexedPanel, you have to keep track
		 * of index-to-Widget relationships yourself, since {@link Wizard} uses
		 * <code>getContent().showWidget(int index)</code>.
		 * 
		 * @return implementation of {@link HasIndexedWidgets} for the {@link Wizard}'s
		 *         main content area
		 */
		HasIndexedWidgets getContent();

		/**
		 * Returns the Display's {@link HandlerFactory}, which provides common event
		 * handlers for user created {@link Wizard}s and {@link WizardPage}s.
		 * 
		...
		/**
		 * Indicate to the view that work is being done. The default view shows an
		 * animated "working" icon.
		 */
		void startProcessing();

		/**
		 * Indicate to the view that work is no longer being done (used after calling
		 * {@link #startProcessing()}.
		 */
		void stopProcessing();

		/**
		 * {@link Wizard} is a composite that. <code>asWidget()</code> returns the
		 * Widget that the Wizard will wrap using <code>initWidget</code>.
		 * 
	...
	/**
	 * Defines the four types of Buttons a {@link Wizard} can contain. Used in the
	 * button methods.
	 * 
	...
	public enum ButtonType {
		BUTTON_CANCEL, BUTTON_FINISH, BUTTON_PREVIOUS, BUTTON_NEXT
	}
	...
	/**
	 * Construct a new Wizard, specifying a caption and {@link WizardContext}.
	 * 
	...
	/**
	 * Construct a new Wizard, specifying a caption, {@link WizardContext}, and
	 * custom {@link Display}.
	 * 
	 * @param caption the caption for the Wizard
	 * @param context the contet for the Wizard
	...
	/**
	 * Initializes the UI by calling <code>initWidget</code> on the view and does
	 * universal UI setup.
	 */
	...
	/**
	 * Convenience method to get the {@link PageID} of the current
	 * {@link WizardPage}.
	 * 
	 * @return the {@link PageID} of the current {@link WizardPage}, or
	 ...
	 * 
	 * @param pageId         the {@link PageID} of the page to display
	 * @param fireBeforeNext whether or not to call
	...
	public void showPage(WizardPage.PageID pageId, boolean fireBeforeNext, boolean fireAfterNext,
		...
		final WizardPage<C> page = getPageByPageID(pageId);
		if (page != null)
			showPage(pages.indexOf(page), fireBeforeNext, fireAfterNext, fireBeforeShow);
	}

	/**
	 * Show a page based on its numeric index in the list, firing all
	 * {@link WizardPage} hooks.
	 * 
	...
	/**
	 * Show a page based on its numeric index in the list, firing only specific
	 * {@link WizardPage} hooks. Contains the logic for actually showing a specific
	 * page--all other page changing methods should call this method.
	 * 
	 * @param num            the index of the page
	 * @param fireBeforeNext whether or not to call
	 *                       {@link WizardPage#beforeNext(NavigationEvent)}
	 ...
	 * @param fireBeforeShow whether or not to call {@link WizardPage#beforeShow()}
	 */
	protected void showPage(int num, boolean fireBeforeNext, boolean fireAfterNext, boolean fireBeforeShow) {
		...
		final WizardPage.PageID newDestination = event.getDestinationPage();
		if (newDestination != targetPage.getPageID()) {
			// beforeNext() changed the destination page.
			showPage(event.getDestinationPage(), event.getFireBeforeNext(), event.getFireAfterNext(),
					event.getFireBeforeShow());
			return;
		}
		...
		final Integer targetPageNum = num;
	 ...
	 * @param fireBeforeShow whether or not to call {@link WizardPage#beforeShow()}
	 */
	public void showNextPage(boolean fireBeforeNext, boolean fireAfterNext, boolean fireBeforeShow) {
...
//        showPage(getPreviousPageID(), false, false, true);
		showPage(getPreviousPageID(), true, true, true);
	}
	...
	public WizardPage.PageID getNextPageID() {
		final PageID nextPage = getLinkManager().getForwardLinkDestination(getCurrentPageID());
		if (nextPage == null) {
	 ...
	 * 
	 * @return the {@link PageID} of the previous page, or <code>null</code> if
	 *         none.
	 */
	public WizardPage.PageID getPreviousPageID() {
		final PageID lastPage = getLinkManager().getReverseLinkDestination(getCurrentPageID());
		if (lastPage == null) {
	...
	/**
	 * This method enables or disables the previous and next buttons based on the
	 * current page position and whether there are any pages before or after the
	 * current page. The user can request that this functionality be disabled for
	 * the <em>next page change</em> by passing true to
	 * {@link #setButtonOverride(boolean)}.
	 * <p>
	 * Adding or removing page links via the Wizard's {@link WizardPageLinkManager}
	 * will not immediately cause the button states to readjust; if the buttons need
	 * to be readjusted immediately, you must call the method manually.
	 */
	...
	public WizardPage<C> getPageByPageID(WizardPage.PageID pageId) {
		...
		while (iter.hasNext()) {
			...
			final WizardPage.PageID iterId = page.getPageID();
			if (iterId == pageId) {
				return page;
			}
		}
	...
	/**
	 * Returns the Wizard's {@link WizardPageLinkManager}, which manages custom
	 * links between pages of the Wizard.
	 * 
	 ...
	 * 
	 * @return <code>true</code> if lazy loading is enabled, <code>false</code>
	 *         otherwise
	 * @see LazyComposite
	 */
	 ...
	 * 
	 * @param useLazyPageLoading <code>true</code> to enable lazy loading,
	...
	public void setButtonVisible(ButtonType button, boolean visible) {
		final HasWizardButtonMethods gwtButton = getButton(button);
		if (gwtButton != null)
	...
	/**
	 * Get one of the Wizard's button's {@link HasWizardButtonMethods}
	 * implementation.
	 * 
	 * @param type the {@link ButtonType} of the button to return
	 *             {@link HasWizardButtonMethods} implementation for
	...
	/**
	 * Turning on the Wizard's button override means that the Wizard will not
	 * attempt to automatically calculate button states based on the current page's
	 * position after the next page change. This is used for
	 * {@link WizardPage#beforeShow()} and other {@link WizardPage} callbacks that
	 * need to manually modify the button state. The {@link WizardPage} must
	 * <strong>completely</strong> handle the buttons' states after setting this to
	 * <code>true</code>. This value is reset to <code>false</code> after each page
	 * change.
	 * 
	 * @param override whether or not to override the button state detection during
	...
	/**
	 * Indicates whether or not the Wizard is busy processing data (e.g., busy with
	 * network communication, etc). This method simply calls
	 * {@link Display#startProcessing()} and {@link Display#stopProcessing()} as
	 * appropriate. The status of the Wizard's buttons are not changed.
	 * 
