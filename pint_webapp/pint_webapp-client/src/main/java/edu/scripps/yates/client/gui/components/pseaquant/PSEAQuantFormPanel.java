...
public class PSEAQuantFormPanel extends PSEAQuantFormAbstractPanel {
	...
	private final edu.scripps.yates.ProteinRetrievalServiceAsync service = GWT.create(ProteinRetrievalService.class);
	...
	public PSEAQuantFormPanel(final Collection<String> projectTags) {
		...
		final FlowPanel headerFlowPanel = new FlowPanel();
		...
		final Label lblPseaquantInterfaceFrom = new Label("PSEA-Quant interface from PINT");
		...
		final Label lblNewLabel_4 = new Label(
		...
		final Label lblNewLabel_5 = new Label("PSEA-Quant server URL: ");
		...
		final FlexTable grid_6 = new FlexTable();
		flowPanel.add(grid_6);

		final Label lblEnterYourEmail = new Label("Enter your email here:");
		...
		final Label lblYouWillReceive = new Label(
		...
		final FlowPanel step1FlowPanel = new FlowPanel();
		...
		final Label lblNewLabel = new Label("1. Select quantification type:");
		...
		for (final PSEAQuantQuantType quantType : PSEAQuantQuantType.values()) {
			final RadioButton radio = new RadioButton(PSEAQuantQuantType.getParameterName(), quantType.getText());
		...
		final FlowPanel step3FlowPanel = new FlowPanel();
		step3FlowPanel.setStyleName("PSEAQuantPanel-Step");
		mainFlowPanel.add(step3FlowPanel);

		final FlexTable replicatesGrid = new FlexTable();
		step3FlowPanel.add(replicatesGrid);

		final Label lblReplicateSelection = new Label("3. Replicate selection:");
		...
		final Label lblNumerOfReplicates = new Label("Numer of replicates:");
		...
		final FlowPanel step4FlowPanel = new FlowPanel();
		step4FlowPanel.setStyleName("PSEAQuantPanel-Step");
		mainFlowPanel.add(step4FlowPanel);

		final Grid grid_1 = new Grid(3, 1);
		step4FlowPanel.add(grid_1);

		final Label lblNumberOf = new Label("4. Number of samplings for statistical significance assessment:");
		lblNumberOf.setStyleName("PSEAQuantPanel-Title-Label");
		grid_1.setWidget(0, 0, lblNumberOf);

		final Label lblrecommendedValue = new Label("(Recommended value: 10,000)");
		...
		final FlowPanel step5FlowPanel = new FlowPanel();
		step5FlowPanel.setStyleName("PSEAQuantPanel-Step");
		mainFlowPanel.add(step5FlowPanel);

		final FlexTable grid_2 = new FlexTable();
		step5FlowPanel.add(grid_2);

		final Label lblAnnotationDatabase = new Label("5. Annotation database:");
		...
		row = 1;
		for (final PSEAQuantAnnotationDatabase pseaQuantAnnotationDatabase : annDBs) {
			final RadioButton radio = new RadioButton(PSEAQuantAnnotationDatabase.getParameterName(),
		...
		final FlowPanel step6FlowPanel = new FlowPanel();
		step6FlowPanel.setStyleName("PSEAQuantPanel-Step");
		mainFlowPanel.add(step6FlowPanel);

		final FlexTable grid_3 = new FlexTable();
		step6FlowPanel.add(grid_3);

		final Label lblSampingProcedure = new Label("6. Samping procedure:");
		...
		final Label lblNewLabel_3 = new Label(
		...
		final Grid grid_4 = new Grid(1, 2);
		...
		final FlowPanel step7flowPanel = new FlowPanel();
		step7flowPanel.setStyleName("PSEAQuantPanel-Step");
		mainFlowPanel.add(step7flowPanel);

		final Grid grid_5 = new Grid(2, 1);
		step7flowPanel.add(grid_5);

		final Label lblAnnotationDatabase_1 = new Label("7. Litterature bias:");
	...
	private void checkInputParameters() {
		...
		if (invalidOrganismError || organism == null) {
			...
			for (final PSEAQuantSupportedOrganism pseaQuantSupportedOrganism : values) {
	...
	protected void createReplicatePickerPanel() {
		final ReplicatePickerPanel panel = new ReplicatePickerPanel(projectTags);
	...
	private void loadRatios() {
		service.getRatioDescriptorsFromProjects(projectTags, new AsyncCallback<List<RatioDescriptorBean>>() {
			...
			public void onSuccess(List<RatioDescriptorBean> result) {
				...
				for (final RatioDescriptorBean ratioDescriptorBean : result) {
	...
	public RatioDescriptorBean getRatioDescriptor() {
		for (final RadioButton radio : quantTypeRadioButtons) {
		...
		for (final String projectTag : projectTags) {
			service.getOrganismsByProject(projectTag, new AsyncCallback<Set<OrganismBean>>() {
				...
				public void onSuccess(Set<OrganismBean> result) {
					...
					final StringBuilder invalidOrganismsString = new StringBuilder();
					for (final OrganismBean organismBean : result) {
						...
						if (organismName != null) {
							...
							final RadioButton radioButton = new RadioButton("Organism", radioLabel);
					...
					if (numValidOrganisms == 0) {
						...
						final StringBuilder supportedOrganisms = new StringBuilder();
						for (final PSEAQuantSupportedOrganism organism : PSEAQuantSupportedOrganism.values()) {
	...
	public PSEAQuantAnnotationDatabase getAnnotationDatabase() {
		for (final RadioButton radio : annotationDBRadioButtons) {
	...
	public PSEAQuantSupportedOrganism getOrganism() {
		for (final RadioButton radioButton : organismsRatioButtons) {
			if (radioButton.getValue() != null && radioButton.getValue()) {
				final String text = radioButton.getText().trim();
				int ncbiID = -1;
	...
	public List<PSEAQuantReplicate> getReplicates() {
		final List<PSEAQuantReplicate> ret = new ArrayList<PSEAQuantReplicate>();

		for (final ReplicatePickerPanel replicatePicker : replicatePickerList) {
			final PSEAQuantReplicate replicate = new PSEAQuantReplicate();
			if (replicatePicker.isConditionsSelected()) {
				...
				for (final String projectTag : selectedConditionsByProject.keySet()) {
				...
				for (final String projectTag : selectedMSRunsByProject.keySet()) {
					final Set<String> msRunsIDs = selectedMSRunsByProject.get(projectTag);
					for (final String msRunID : msRunsIDs) {
