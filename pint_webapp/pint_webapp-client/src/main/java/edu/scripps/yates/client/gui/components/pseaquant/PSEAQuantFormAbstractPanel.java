...
public abstract class PSEAQuantFormAbstractPanel extends InitializableComposite implements PSEAQuantForm {
	protected final edu.scripps.yates.ProteinRetrievalServiceAsync service = GWT.create(ProteinRetrievalService.class);
		...
		service.sendPSEAQuantQuery(getEmail(), getOrganism(), replicates, getRatioDescriptor(), getNumberOfSamplings(),
				...
				new AsyncCallback<PSEAQuantResult>() {
					...
					public void onSuccess(PSEAQuantResult result) {
						loadingDialog.hide();
						final WindowBox window = new WindowBox(true, true);
						window.setText("PSEA-Quant results");
						window.setAnimationEnabled(true);
						if (result.getErrorMessage() != null) {
							StatusReportersRegister.getInstance().notifyStatusReporters(result.getErrorMessage());
							return;
						}
						if (resultscontainer != null) {
	...
	private void showError(String errorMessage) {
		final WindowBox window = new WindowBox(false, true, true, true, true);
