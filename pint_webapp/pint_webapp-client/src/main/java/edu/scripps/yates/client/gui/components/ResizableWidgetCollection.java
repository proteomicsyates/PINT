...
public class ResizableWidgetCollection implements ResizeHandler, Iterable<ResizableWidget> {
	...
	static class ResizableWidgetInfo {
		...
		private final ResizableWidget widget;
		...
		public boolean updateSizes() {
			final int offsetWidth = widget.getElement().getOffsetWidth();
			final int offsetHeight = widget.getElement().getOffsetHeight();
			final int clientWidth = widget.getElement().getClientWidth();
			final int clientHeight = widget.getElement().getClientHeight();
	...
	/**
	 * Get the globally accessible {@link ResizableWidgetCollection}. In most cases,
	 * the global collection can be used for all {@link ResizableWidget}s.
	 * 
	 ...
	 */
	private final Map<ResizableWidget, ResizableWidgetInfo> widgets = new HashMap<ResizableWidget, ResizableWidgetInfo>();
	...
	public void checkWidgetSize() {
		for (final Map.Entry<ResizableWidget, ResizableWidgetInfo> entry : widgets.entrySet()) {
			final ResizableWidget widget = entry.getKey();
			final ResizableWidgetInfo info = entry.getValue();
...
//				});
	...
	public void updateWidgetSize(ResizableWidget widget) {
		...
		final ResizableWidgetInfo info = widgets.get(widget);
		if (info != null) {
			info.updateSizes();
		}
	}
