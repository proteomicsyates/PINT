...
public class ProgressBar extends Widget implements ResizableWidget {
	 ...
	 */
	private final com.google.gwt.dom.client.Element barElement;
	 ...
	 * The percent will always be between 0.0 - 1.0.
	 * 
	...
	public double getPercent() {
		...
		final double percent = (curProgress - minProgress) / (maxProgress - minProgress);
	...
	public void onResize(int width, int height) {
		if (textVisible) {
			final int textWidth = textElement.getOffsetWidth();
			final int left = (width / 2) - (textWidth / 2);
	...
	public void setTextVisible(boolean isVisible) {
		...
		if (textVisible) {
			textElement.getStyle().setDisplay(Style.Display.INLINE);
			redraw();
		} else {
	...
	/**
	 * Generate the text to display within the progress bar. Override this function
	 * to change the default progress percent to a more informative message, such as
