...
import edu.scripps.yates.shared.columns.comparator.ClientPeptideComparator;
...
public class PeptideTextColumn extends CustomTextColumn<PeptideBean> implements MyIdColumn<PeptideBean> {
	...
	public PeptideTextColumn(ColumnName columnName, boolean visibleState, Header<?> header, Header<String> footer) {
		...
		comparator = new ClientPeptideComparator(columnName);
	...
	public PeptideTextColumn(ColumnName columnName, boolean visibleState, Header<?> header, Header<String> footer,
		...
		setSortable(true);
		this.columnName = columnName;
		comparator = new ClientPeptideComparator(columnName, conditionName, amountType, projectTag);
		defaultWidth = getDefaultWidth(columnName);
	...
	public PeptideTextColumn(ColumnName columnName, boolean visibleState, Header<?> header, Header<String> footer,
		...
		setSortable(true);
		this.columnName = columnName;
		comparator = new ClientPeptideComparator(columnName, condition1Name, condition2Name, projectTag, ratioName);
		defaultWidth = getDefaultWidth(columnName);
	...
	public PeptideTextColumn(ColumnName columnName, boolean visibleState, Header<?> header, Header<String> footer,
		...
		setSortable(true);
		this.columnName = columnName;
		comparator = new ClientPeptideComparator(columnName, scoreName);
		defaultWidth = getDefaultWidth(columnName);
		conditionName = null;
		condition2Name = null;
		amountType = null;
	...
	public void render(Context context, PeptideBean pep, SafeHtmlBuilder sb) {
		...
		case PEPTIDE_SEQUENCE:
			...
			sb.append(ClientSafeHtmlUtils.getRichPeptideSequence(pep.getSequence(), pep.getPtms()));
		...
		case PEPTIDE_RATIO:
			sb.append(template.startToolTip("Ratio type: " + ratioName + "\nConditions: " + conditionName + " / "
					+ condition2Name + "\nValue: " + ClientDataUtil.getRatioStringByConditions(pep, conditionName,
		...
		case PEPTIDE_PTM:
			...
			break;
		default:
			sb.append(template.startToolTip(ClientDataUtil.getPeptideColumnValue(columnName, pep, conditionName,
