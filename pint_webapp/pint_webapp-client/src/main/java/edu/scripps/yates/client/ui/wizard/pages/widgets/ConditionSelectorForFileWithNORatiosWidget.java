...
public class ConditionSelectorForFileWithNORatiosWidget extends AbstractConditionSelectorForFileWidget {
	...
	public ConditionSelectorForFileWithNORatiosWidget(PintContext context, FileTypeBean file,
		...
		leftTable.getFlexCellFormatter().setColSpan(0, 0, 4);
		...
		leftTable.getFlexCellFormatter().setColSpan(1, 0, 4);
		...
		leftTable.getFlexCellFormatter().setColSpan(2, 0, 4);
		//
		...
		if (sheetName != null) {
			...
			conditionsAssociatedWithFile = PintImportCfgUtil
					.getConditionsAssociatedWithFile(context.getPintImportConfiguration(), file.getId(), sheetName);
		}
	...
	private void addAssociatedCondition(String conditionID) {
		...
		if (condition.getSampleRef() != null) {
			...
			final SampleTypeBean sample = PintImportCfgUtil.getSample(context.getPintImportConfiguration(),
					condition.getSampleRef());
			final Label label9 = new Label("-");
			label9.setTitle("Sample '" + condition.getSampleRef() + "' is not labelled");
			label9.setStyleName(WizardStyles.WizardDraggableLabelFixed);
			leftTable.setWidget(rowForNewCondition, 2, label9);
			if (sample.getLabelRef() != null) {
				label9.setText(sample.getLabelRef());
				horizontalAlignment2 = HasHorizontalAlignment.ALIGN_LEFT;
			}
		...
		leftTable.getFlexCellFormatter().setHorizontalAlignment(rowForNewCondition, 1, horizontalAlignment1);
		leftTable.getFlexCellFormatter().setHorizontalAlignment(rowForNewCondition, 2, horizontalAlignment2);
		...
		leftTable.setWidget(rowForNewCondition, 3, deleteButton);
		leftTable.getFlexCellFormatter().setHorizontalAlignment(rowForNewCondition, 3,
