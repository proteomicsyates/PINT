...
import edu.scripps.yates.shared.columns.comparator.ClientProteinComparator;
...
public class ProteinTextColumn extends CustomTextColumn<ProteinBean> implements MyIdColumn<ProteinBean> {
	...
	public ProteinTextColumn(ColumnName columnName, boolean visibleState, Header<?> header, Header<String> footer,
		...
		comparator = new ClientProteinComparator(columnName, scoreName);
	...
	public ProteinTextColumn(ColumnName columnName, boolean visibleState, Header<?> header, Header<String> footer) {
		...
		setSortable(true);
		this.columnName = columnName;
		comparator = new ClientProteinComparator(columnName);
		defaultWidth = getDefaultWidth(columnName);
		conditionName = null;
		condition2Name = null;
		this.scoreName = null;
	...
	public ProteinTextColumn(ColumnName columnName, boolean visibleState, Header<?> header, Header<String> footer,
		...
		setSortable(true);
		this.columnName = columnName;
		comparator = new ClientProteinComparator(columnName, conditionName, amountType, projectTag);
		defaultWidth = getDefaultWidth(columnName);
	...
	public ProteinTextColumn(ColumnName columnName, boolean visibleState, Header<?> header, Header<String> footer,
		...
		setSortable(true);
		this.columnName = columnName;
		comparator = new ClientProteinComparator(columnName, condition1Name, condition2Name, projectTag, ratioName);
		defaultWidth = getDefaultWidth(columnName);
	...
	public void render(Context context, ProteinBean p, SafeHtmlBuilder sb) {
		...
		case REACTOME_ID_LINK:
			...
			break;
		default:
			sb.append(template.startToolTip(ClientDataUtil.getProteinColumnValue(columnName, p, conditionName,
