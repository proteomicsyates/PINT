...
public class PSMTextColumn extends CustomTextColumn<PSMBean> implements MyIdColumn<PSMBean> {
	...
	public PSMTextColumn(ColumnName columnName, boolean visibleState, Header<?> header, Header<String> footer) {
		...
		comparator = new ClientPSMComparator(columnName);
	...
	public PSMTextColumn(ColumnName columnName, boolean visibleState, Header<?> header, Header<String> footer,
		...
		setSortable(true);
		this.columnName = columnName;
		comparator = new ClientPSMComparator(columnName, conditionName, amountType, projectTag);
		defaultWidth = getDefaultWidth(columnName);
	...
	public PSMTextColumn(ColumnName columnName, boolean visibleState, Header<?> header, Header<String> footer,
		...
		setSortable(true);
		this.columnName = columnName;
		comparator = new ClientPSMComparator(columnName, condition1Name, condition2Name, projectTag, ratioName);
		defaultWidth = getDefaultWidth(columnName);
	...
	public PSMTextColumn(ColumnName columnName, boolean visibleState, Header<?> header, Header<String> footer,
		...
		setSortable(true);
		this.columnName = columnName;
		comparator = new ClientPSMComparator(columnName, scoreName);
		defaultWidth = getDefaultWidth(columnName);
		conditionName = null;
		condition2Name = null;
		amountType = null;
	...
	public void render(Context context, PSMBean psm, SafeHtmlBuilder sb) {
		...
		case PEPTIDE_SEQUENCE:
			...
			sb.append(ClientSafeHtmlUtils.getRichPeptideSequence(psm.getSequence(), psm.getPtms()));
		...
		case PSM_RATIO:
			sb.append(template.startToolTip("Ratio type: " + ratioName + "\nConditions: " + conditionName + " / "
					+ condition2Name + "\nValue: " + ClientDataUtil.getRatioStringByConditions(psm, conditionName,
		...
		case PEPTIDE_PTM:
			...
			break;
		default:
			sb.append(template.startToolTip(ClientDataUtil.getPSMColumnValue(columnName, psm, conditionName,
