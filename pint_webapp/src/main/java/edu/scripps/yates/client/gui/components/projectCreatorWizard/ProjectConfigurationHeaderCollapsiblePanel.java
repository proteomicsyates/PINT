...
public class ProjectConfigurationHeaderCollapsiblePanel extends Composite implements RepresentsDataObject {
	...
	public ProjectConfigurationHeaderCollapsiblePanel(PintImportCfgBean pintImportCfgTypeBean,
		...
		ToggleButton controlButton = new ToggleButton("(click to pin)", "(click to collapse)");
		...
		FlowPanel uploadIconFlowPanel = new FlowPanel();
		...
		Label lblSaveImportConfiguration = new Label("Save import cfg on server:");
		...
		Label lblNewLabel_1 = new Label("Submit data to server:");
		...
		Label lblDownloadImportConfiguration = new Label("Download import cfg file:");
		...
		Label lblProjectConfiguration = new Label("Project definition");
		...
		Label lblProjectShortName = new Label("Project tag:");
		String projectTagTooltip = "This tag is going to be the idenfier of the project.\nIt is limited to 15 characters.";
		...
		Label lblNewLabel = new Label("Project name:");
		...
		Label lblProjectDescription = new Label("Project description:");
		...
		FlowPanel flowPanel_8 = new FlowPanel();
		...
		InlineLabel nlnlblNewInlinelabel_1 = new InlineLabel("Experimental condition(s):");
		...
		InlineLabel lblRatiosBetweenConditions = new InlineLabel("Ratio(s) between conditions:");
		...
		InlineLabel nlnlblNewInlinelabel_2 = new InlineLabel("Sample(s):");
		...
		InlineLabel nlnlblNewInlinelabel_3 = new InlineLabel("MS run(s):");
		...
		InlineLabel nlnlblNewInlinelabel_7 = new InlineLabel("Input file(s):");
		...
		InlineLabel nlnlblNewInlinelabel_8 = new InlineLabel("Server connection(s):");
		...
		InlineLabel nlnlblNewInlinelabel_4 = new InlineLabel("Sample origin(s):");
		...
		InlineLabel nlnlblNewInlinelabel_5 = new InlineLabel("Organism(s):");
		...
		InlineLabel nlnlblNewInlinelabel_6 = new InlineLabel("Label(s):");
	...
	private void keepRelatedObjects(PintImportCfgTypeBean pintImportCfgTypeBean) {
		...
		if (project != null && project.getExperimentalDesign() != null) {
			ExperimentalDesignTypeBean experimentalDesignTypeBean = project.getExperimentalDesign();
			if (experimentalDesignTypeBean.getSampleSet() != null) {
				for (SampleTypeBean sampleTypeBean : experimentalDesignTypeBean.getSampleSet().getSample()) {
					addSample(sampleTypeBean);
				}
			}
			if (experimentalDesignTypeBean.getLabelSet() != null) {
				for (LabelTypeBean labelTypeBean : experimentalDesignTypeBean.getLabelSet().getLabel()) {
					addLabel(labelTypeBean);
				}
			}
			if (experimentalDesignTypeBean.getOrganismSet() != null) {
				for (OrganismTypeBean organismTypeBean : experimentalDesignTypeBean.getOrganismSet().getOrganism()) {
					addOrganism(organismTypeBean);
				}
			}
			if (experimentalDesignTypeBean.getTissueSet() != null) {
				for (TissueTypeBean tissueTypeBean : experimentalDesignTypeBean.getTissueSet().getTissue()) {
	...
	public void addConditions(Collection<ExperimentalConditionTypeBean> conditions) {
		for (ExperimentalConditionTypeBean experimentalConditionBean : conditions) {
	...
	public void addMSRun(Collection<MsRunTypeBean> msRuns) {
		for (MsRunTypeBean msRunBean : msRuns) {
			addMSRun(msRunBean);
		}
	}
	...
	public void addRatio(Collection<RatioDescriptorTypeBean> ratios) {
		for (RatioDescriptorTypeBean ratioBean : ratios) {
			addRatio(ratioBean);
		}
	}
	...
	public List<ExcelAmountRatioTypeBean> getPeptideExcelRatios() {
		...
		if (pintImportCfgTypeBean != null && pintImportCfgTypeBean.getProject() != null
			...
			RatiosTypeBean ratiosType = pintImportCfgTypeBean.getProject().getRatios();
			// peptide ratios
	...
	private RatioDescriptorTypeBean getRatio(String condition1ID, String condition2ID) {
		RatioDescriptorTypeBean ratio = new RatioDescriptorTypeBean();
	...
	public void removeSample(SampleTypeBean sampleToRemove) {
		...
		for (ExperimentalConditionTypeBean condition : pintImportCfgTypeBean.getProject().getExperimentalConditions()
	...
	public void removeLabel(LabelTypeBean labelToRemove) {
		...
		for (SampleTypeBean sample : pintImportCfgTypeBean.getProject().getExperimentalDesign().getSampleSet()
				.getSample()) {
	...
	public List<FileNameWithTypeBean> getFileNamesWithTypes() {
		List<FileNameWithTypeBean> ret = new ArrayList<FileNameWithTypeBean>();
		for (DataSourceBean dataSourceBean : getDataSources()) {
			...
			if (dataSourceBean.getServer() == null && dataSourceBean.getFormat() != null) {
				...
				FileNameWithTypeBean bean = new FileNameWithTypeBean();
	...
	public List<RemoteFileWithTypeBean> getRemoteFileWithTypeBeans() {
		List<RemoteFileWithTypeBean> ret = new ArrayList<RemoteFileWithTypeBean>();
		for (DataSourceBean dataSourceBean : getDataSources()) {
			...
			if (dataSourceBean.getServer() != null && dataSourceBean.getFormat() != null
				...
				RemoteFileWithTypeBean bean = new RemoteFileWithTypeBean();
				bean.setId(dataSourceBean.getId());
				bean.setFileName(dataSourceBean.getFileName());
				bean.setFileFormat(dataSourceBean.getFormat());
