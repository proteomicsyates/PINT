...
public class FileTypeBeanAdapter implements Adapter<FileTypeBean> {
	...
	private FileTypeBean adaptFromFileTypeBean() {
		...
		if (fileType.getFormat() != null)
			ret.setFormat(new FormatTypeBeanAdapter(fileType.getFormat())
					.adapt());
		...
		if (fileType.getSheets() != null) {
			final SheetsTypeBean sheets = new SheetsTypeBeanAdapter(
					fileType.getSheets()).adapt();
		...
		if (fileType.getFastaDigestion() != null) {
			ret.setFastaDigestion(new FastaDigestionBeanAdapter(fileType
					.getFastaDigestion()).adapt());
		}
		return ret;
	}
}
