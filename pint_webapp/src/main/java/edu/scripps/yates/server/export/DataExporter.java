...
public class DataExporter {
	...
	public static File existsFileProteinsFromProjects(List<String> projectTags) throws PintException {
		...
		final String projectsStringTag = SharedDataUtils.getProjectTagCollectionKey(projectTags);
			...
			if (columnWithOrder.getColumn() == ColumnName.PEPTIDE_AMOUNT) {
				...
				for (final String projectTag : projectTags) {
					...
					if (ratioDescriptors != null) {
						...
						for (final RatioDescriptorBean ratioDescriptor : ratioDescriptors) {
							if (actualprojectTag.equals(projectTag)) {
								final String ratioStringByConditions = peptideBean.getRatioStringByConditions(
										ratioDescriptor.getCondition1Name(), ratioDescriptor.getCondition2Name(),
										ratioDescriptor.getProjectTag(), ratioDescriptor.getRatioName(), false, false);
								String parsedValue = parseValue(ratioStringByConditions);
								if ("".equals(parsedValue)) {
				...
				for (final String projectTag : projectTags) {
					...
					if (ratioDescriptors != null) {
						...
						for (final RatioDescriptorBean ratioDescriptor : ratioDescriptors) {
							final String psmScoreName = ratioDescriptor.getPsmScoreName();
							if (actualprojectTag.equals(projectTag) && psmScoreName != null) {
								final String ratioStringByConditions = peptideBean.getRatioScoreStringByConditions(
										ratioDescriptor.getCondition1Name(), ratioDescriptor.getCondition2Name(),
										ratioDescriptor.getProjectTag(), ratioDescriptor.getRatioName(), psmScoreName,
										false, false);
								String parsedValue = parseValue(ratioStringByConditions);
				...
				if (ptmScoreNames.isEmpty()) {
					...
					for (final String ptmScoreName : ptmScoreNames) {
						final String ptmScoreString = peptideBean.getPTMScoreString(ptmScoreName);
						if (ptmScoreString != null && !"".equals(ptmScoreString)) {
							fw.write(ptmScoreString + TAB);
						} else {
							fw.write(EMPTY);
				...
				final String value = PeptideColumns.getInstance().getValue(columnWithOrder.getColumn(), peptideBean,
	...
	public static File exportProteins(Collection<ProteinBean> proteins, String queryText, boolean psmCentric)
		...
		try {
			...
			final Map<String, List<String>> conditionsByProject = SharedDataUtils.getConditionsByProjects(proteins);
			...
			List<String> peptideScoreNames = null;
			if (psmCentric) {
				psmScoreNames = SharedDataUtils.getPSMScoreNamesFromProteins(proteins);
			} else {
				peptideScoreNames = SharedDataUtils.getPeptideScoreNamesFromProteins(proteins);
			}
