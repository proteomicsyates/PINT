...
public class ImportCfgFileParserUtil {
	...
	static {
		try {
			jaxbContext = JAXBContext
					.newInstance("edu.scripps.yates.excel.proteindb.importcfg.jaxb");
	 ...
	 */
	public static PintImportCfg getPintImportFromFile(File fileCfg)
			throws PintException {
		try {
			Unmarshaller marshaller = jaxbContext.createUnmarshaller();
			final PintImportCfg unmarshal = (PintImportCfg) marshaller
					.unmarshal(fileCfg);
			return unmarshal;
		} catch (JAXBException e) {
			e.printStackTrace();
			throw new PintException(e.getMessage(),
	 ...
	 *             jodID
	 */
	public static PintImportCfg getPintImportCfgFromJobID(int jobID)
			throws PintException {
		log.info("getting import process id '" + jobID + "'");
		final File projectCfgFileByImportProcessID = FileManager
				.getProjectCfgFileByImportProcessID(jobID);
		...
		final String message = "Import process ID: '"
				+ jobID
				+ "' doesn't exist or there was a problem retrieving the import process";
		log.warn(message);
		throw new PintException(message,
				PINT_ERROR_TYPE.IMPORT_PROCESS_ID_NOT_FOUND);
	}
	...
	public static File saveFileCfg(PintImportCfg pintImportCfg,
		...
		try {
			...
			marshaller.setProperty(Marshaller.JAXB_FORMATTED_OUTPUT,
					new Boolean(true));
			File xmlFile = FileManager.getProjectXmlFile(projectFilesPath,
					pintImportCfg.getProject().getTag() + ".xml");
